...
export default function EditDetail({
  ...
  const { mutate: changePassword, isPending: isLoadingChangePassword } = UserHook.useChangePassword();
  ...
  const onSubmitPassword: SubmitHandler<ChangePasswordInputs> = async (data) => {
    try {
  ...
  useEffect(() => {
    if (user) {
      ...
      setValue("day_of_birth", user.day_of_birth ? formatDate(user.day_of_birth) : "");
    }
  }, [user, setValue]);
  ...
  // --- Common Input Styles ---
  const inputClass = "w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all duration-200 text-gray-800 text-sm";
    ...
    <div className="flex flex-col gap-8 mt-5 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        className={inputClass}
                    />
                    {errors.day_of_birth && <p className={errorClass}>{errors.day_of_birth.message}</p>}
                </div>
        ...
        <div className={`
            ...
            ${isEditingPassword ? 'border-blue-200 shadow-blue-100' : 'border-gray-100'}
        `}>
            <div className="flex items-center justify-between mb-2 pb-4 border-b border-gray-100">
                <div className="flex items-center gap-2">
                    <LockIcon className={`w-5 h-5 ${isEditingPassword ? 'text-blue-600' : 'text-gray-400'}`} />
                    <h3 className="font-bold text-gray-800 text-lg">Bảo mật</h3>
                </div>
                
                {/* Switch Toggle nhỏ gọn */}
                <div className="flex items-center gap-2">
                    <label htmlFor="edit-password-toggle" className="text-sm text-gray-500 cursor-pointer select-none">
                    ...
                    <input
                        ...
                        className="toggle toggle-primary toggle-sm" // Nếu dùng DaisyUI, hoặc checkbox thường
                    />
                </div>
            </div>
            ...
            <form
                ...
                className={`flex flex-col gap-4 relative transition-opacity duration-200 ${!isEditingPassword ? 'opacity-50 select-none' : 'opacity-100'}`}
            >
                {/* Overlay chặn click khi không edit */}
                {!isEditingPassword && <div className="absolute inset-0 z-10 cursor-not-allowed" />}

                <div>
                    <label htmlFor="old-password" className={labelClass}>Mật khẩu cũ</label>
                    ...
                    {passwordErrors.oldPassword && <p className={errorClass}>{passwordErrors.oldPassword.message}</p>}
                </div>

                <div>
                    <label htmlFor="new-password" className={labelClass}>Mật khẩu mới</label>
                    <input
                        id="new-password"
                        type="password"
                        disabled={!isEditingPassword}
                ...
                <div>
                    <label htmlFor="confirm-password" className={labelClass}>Xác nhận mật khẩu</label>
                    <input
                        id="confirm-password"
                        type="password"
                        disabled={!isEditingPassword}
                ...
                <div className="pt-2 mt-auto">
                    <PrimaryButton
                        ...
                        text={isLoadingChangePassword ? "Đang xử lý..." : "Cập nhật mật khẩu"}
                        className="w-full"
                    />
                </div>
            </form>
